<?php 
	...
	$conteudo = !isset($_GET['conteudo'])? "home": !file_exists(str_replace("ç","c",$_GET['conteudo']).".php")?"home":$_GET['conteudo'];
...
<?php 
	foreach($dataMenu as $menu){
		echo '<li class="'.( ('home'==$menu)?'home':'' ).( ($conteudo==$menu)?' active':'' ).'"><a href="index.php?conteudo='.$menu.'">'.ucfirst($menu).'</a></li>';
	}
?>        			
            	</ul>
    	    </div>
			...
			<!-- Conteudo -->
	        <div id="conteudo"><?php require_once(str_replace("ç","c",$conteudo).".php") ?></div>
			<!-- end.Conteudo -->
